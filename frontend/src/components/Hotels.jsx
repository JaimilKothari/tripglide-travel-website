...
//         setDestinations([]); // Fallback to empty array on error
//       });
//   }, []);

//   const handleSearch = (e) => {
//     e.preventDefault();
//     // Navigate to HotelSearch with the form data
...
//                       <button type="button" onClick={() => handleOption("adults", "dec")} className="p-2 bg-gray-200 rounded"><FaMinus /></button>
//                       <span>{adults}</span>
//                       <button type="button" onClick={() => handleOption("adults", "inc")} className="p-2 bg-gray-200 rounded"><FaPlus /></button>
//                     </div>
//                   </div>
//                   <div className="flex justify-between items-center mb-4">
//                     <span>Children</span>
//                     <div className="flex items-center space-x-3">
//                       <button type="button" onClick={() => handleOption("children", "dec")} className="p-2 bg-gray-200 rounded"><FaMinus /></button>
//                       <span>{children}</span>
//                       <button type="button" onClick={() => handleOption("children", "inc")} className="p-2 bg-gray-200 rounded"><FaPlus /></button>
//                     </div>
//                   </div>
//                   <div className="flex justify-between items-center mb-4">
//                     <span>Rooms</span>
//                     <div className="flex items-center space-x-3">
//                       <button type="button" onClick={() => handleOption("rooms", "dec")} className="p-2 bg-gray-200 rounded"><FaMinus /></button>
//                       <span>{rooms}</span>
//                       <button type="button" onClick={() => handleOption("rooms", "inc")} className="p-2 bg-gray-200 rounded"><FaPlus /></button>
//                     </div>
//                   </div>
//                   <button type="button" className="text-blue-600 font-semibold mt-2" onClick={() => setShowGuestOptions(false)}>Done</button>
//                 </div>
//               )}
//             </div>
...
//         </div>
//       </div>
//       {/* Rest of the Hotels page remains unchanged */}
  ...
  const handleOption = (type, operation) => {
    if (type === "adults") {
      if (operation === "inc") {
        ...
        // Calculate required rooms: 2 adults max per room
        const requiredRooms = Math.ceil(newAdults / 2);
        // Ensure children fit: 1 child max per room
        const totalGuests = newAdults + children;
        const minRoomsForGuests = Math.ceil(totalGuests / 3); // Since max 3 guests (2 adults + 1 child) per room
        const newRooms = Math.max(requiredRooms, minRoomsForGuests, rooms);
        setAdults(newAdults);
        setRooms(newRooms);
      } else {
        const newAdults = adults > 1 ? adults - 1 : 1;
        // Recalculate required rooms
        const requiredRooms = Math.ceil(newAdults / 2);
        const totalGuests = newAdults + children;
    ...
    if (type === "rooms") {
      if (operation === "inc") {
        ...
        // Ensure enough rooms for adults (2 per room) and children (1 per room)
        const requiredRoomsForAdults = Math.ceil(adults / 2);
        const requiredRoomsForChildren = children;
        const totalGuests = adults + children;
        const minRoomsForGuests = Math.ceil(totalGuests / 3);
        const minRooms = Math.max(requiredRoomsForAdults, requiredRoomsForChildren, minRoomsForGuests);
        setRooms(Math.max(1, minRooms));
      }
    }
  };
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <button
                        ...
                        className="p-2 bg-gray-200 rounded disabled:opacity-50"
                        disabled={children >= rooms} // Limit children to number of rooms
                      >
                        <FaPlus />
                      </button>
                    </div>
