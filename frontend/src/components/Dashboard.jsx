...
//           const isUpcoming = booking.status === "Upcoming" && new Date(booking.check_in_date) > new Date("2025-04-15");
//           return (
//             <motion.div
//               key={booking.id}
//               variants={itemVariants}
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setSubsError(res.error || "Failed to update profile");
      }
    } catch (err) {
      console.error("Save profile error:", err);
      setError(`Failed to update profile: ${err.message}`);
      ...
      <div className="flex flex-col lg:flex-row flex-1 max-w-7xl mx-auto w-full p-4 gap-6">
        ...
        >
          <div className="flex items-center mb-8">
            ...
            <span className="ml-3 SCRIPT
text-lg font-semibold text-gray-800 truncate">{user.username}</span>
          </div>
          <ul className="space-y-2">
            {[
              { section: "Profile", label: "Profile", icon: <FaUser className="mr-2" /> },
